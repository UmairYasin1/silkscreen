...
namespace HaroonPOSWeb.Controllers
{
    public class ReportController : BaseController
    {
        ...
        public ActionResult ShowPartyPurchaseLedger(string stringValues)
        {
            ...
            var purchaser = db.MaterialInPurchasers.Where(x => x.MatInPurchaserId.Equals(MatInPurchaserIdVal)).FirstOrDefault();
            var month = db.Months.Where(x => x.MonthId.Equals(MonthIdVal)).FirstOrDefault();
            var year = db.Years.Where(x => x.YearId.Equals(YearIdVal)).FirstOrDefault();
            int monthVal = MonthIdVal;
            int yearVal = Convert.ToInt32(year.YearName);
            var purchaseItemList = db.MaterialIns.Where(x => x.MatInPurchaserId.Value.Equals(MatInPurchaserIdVal)
                                    && x.CreateDate.Value.Month.Equals(monthVal)
                                    && x.CreateDate.Value.Year.Equals(yearVal)).ToList();
            ShowPartyPurchaseLedgerModel model = new ShowPartyPurchaseLedgerModel();
            model.PartyName = purchaser.Name;
            model.Month = month.MonthName;
            model.Year = year.YearName.Value;

            List<MaterialInModel> modelMatInList = new List<MaterialInModel>();

            if (purchaseItemList != null)
            {
                foreach (var item in purchaseItemList)
                {
                    modelMatInList.Add(new MaterialInModel()
                    {
            ...
            model.MaterialInModelList = modelMatInList;

            return View(model);
        }

        public ActionResult DownloadPartyPurchaseLedger(string stringValues)
        {
            string currentUrl = Request.Url.Authority;
            currentUrl = currentUrl + "/Report/ShowPartyPurchaseLedger?stringValues=" + stringValues;
            HtmlToPdf converter = new HtmlToPdf();
            PdfDocument doc = converter.ConvertUrl(currentUrl);
        ...
        //#region download Stock Report pdf
